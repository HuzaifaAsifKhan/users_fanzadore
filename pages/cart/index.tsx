...
class CartPage extends PureComponent<IProps> {
  ...
  async purchaseProducts() {
    try {
      ...
      const { deliveryAddress, isApplyCoupon, couponCode } = this.state;
      const { items } = cart;
      if (items && items.length > 0) {
        if (!deliveryAddress) {
          return message.error('Please enter your delivery address');
        }
  ...
  render() {
    const { ui } = this.props;
    const {
      products, isApplyCoupon, coupon, couponCode
    } = this.state;
                ...
                >
                  ...
                  <Col span={24}>
                    ...
                    <Space>
                      <span
                        className={
                          isApplyCoupon ? 'discount-price' : 'initialPrice'
                        }
                      >
                        $
                        {calTotal(products)}
